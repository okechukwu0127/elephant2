...
export default class Dropdown extends PureComponent {
    ...
    UNSAFE_componentWillReceiveProps({value}) {
        if (value !== this.props.value) {
            if (value == null) {
                this.txtInput && this.txtInput.clear();
            }
            this.setState({value});
        }
    }
                    ...
                    if (this.mounted) {
                        ...
                        Animated.timing(opacity, {
                            ...
                            if (this.mounted && 'ios' === Platform.OS) {
                                ...
                                if (
                                    'function' === typeof flashScrollIndicators
                                ) {
    ...
    selectedIndex() {
        ...
        return data.findIndex(
            (item, index) =>
                null != item && value === valueExtractor(item, index),
        );
    }
    ...
    renderBase(props) {
        ...
        title =
            null == title || 'string' === typeof title ? title : String(title);
        return (
            <TextField
                ref={(_ref) => (this.txtInput = _ref)}
                label=""
                labelHeight={0}
                {...props}
                value={title}
                ...
                fontSize={
                    this.props.fontSize ? this.props.fontSize : width / 30
                }
            />
        );
    }
    ...
    renderAccessory() {
        let {baseColor: backgroundColor, arrowSize, arrowColor} = this.props;
        let triangleStyle = {backgroundColor};

        return (
            <Icon
                name="angle-down"
                color={arrowColor ? arrowColor : PRIMARY_COLOR}
                size={arrowSize ? arrowSize : 25}
                style={{marginTop: -2}}
            />
        );
    }
    ...
    renderItem({item, index}) {
        ...
        return (
            <DropdownItem index={index} {...props}>
                <Text
                    style={[styles.item, itemTextStyle ? itemTextStyle : {}]}
                    numberOfLines={1}>
                    {title}
                </Text>
            </DropdownItem>
            ...
            switch (selected) {
                ...
                default:
                    if (selected >= itemCount - tailItemCount) {
                        translateY -=
                            itemSize *
                            (visibleItemCount - (itemCount - selected));
                    } else {
                        translateY -= itemSize;
                    }
            }
